<?php
	...
	function TS_VCSC_Google_Maps_Function ($atts, $content = null) {
		...
		if (($mapfullwidth == "true") && ($mapfullwrapper == "true")) {
			...
			$output .= '<div id="' . $map_id . '" class="ts-map-frame ' . $css_class . ' clearFixMe" data-height="' . $height . '" data-activate="false" data-inline="' . $editor_frontend . '" data-break-parents="' . $breakouts . '" style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px; height: ' . $height . 'px;"></div>';
		if (($mapfullwidth == "true") && ($mapfullwrapper == "true")) {
			$output .= '</div>';
		}
			...
			<script type="text/javascript">
				jQuery(document).ready(function($){
					jQuery('#<?php echo $map_id; ?>').TSGM_Map({
						...
						TSGM_MapStyle:			$<?php echo $mapstyle; ?>,
						TSGM_MapCustom:			'<?php echo $mapstyle; ?>',
