<?php
    ...
    function TS_VCSC_SyncMenu() {
        ...
        add_action('admin_enqueue_scripts', array($VISUAL_COMPOSER_EXTENSIONS, 'TS_VCSC_AdminScripts'));
        if (get_option('ts_vcsc_extend_settings_mainmenu', 1) == 1) {
            $ts_vcsc_main_page = 		        add_menu_page( 		                        "VC Extensions",        "VC Extensions",    	'manage_options', 	'TS_VCSC_Extender', 	'TS_VCSC_PageExtend', 	    TS_VCSC_GetResourceURL('images/logos/ts_vcsc_menu_icon_16x16.png'));
        } else {
        ...
        if (((get_option('ts_vcsc_extend_settings_extended', 0) == 1) && (get_option('ts_vcsc_extend_settings_fontimport', 1) == 1)) || ((get_option('ts_vcsc_extend_settings_extended', 0) == 0))) {
            $ts_vcsc_upload_page = 				add_submenu_page( 	'TS_VCSC_Extender', 	"Import Font",          "Import Font",      	'manage_options', 	'TS_VCSC_Uploader', 	'TS_VCSC_PageUpload');
        }
