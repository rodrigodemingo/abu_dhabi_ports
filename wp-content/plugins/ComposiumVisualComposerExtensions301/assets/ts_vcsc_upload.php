	...
	** TO DO RENAME UPLOADED FILE TO ts-vcsc-custom-pack.zip ** */
	if (isset($_POST['Submit']) && (isset($_FILES['custom_icon_pack']))) {
		...
		*/
		...
		$targetzip 	= $filepath . $filename; 				// target zip file
		...
		if ($movefile) {	
			...
			if ($unzipfile) {
				...
				if (file_exists($dest_path . '/style.css')) {
					...
					<div id="ts-vcsc-async-upload-wrap" class="clearFixMe" style="margin-bottom:0;">
						...
						<div style="width:100%; display: block;">
							...
							<?php if ($custom_package_exists == "true") { ?>
								<br/><br/><span style="color: red;">If you have problems uninstalling the font pack, manually delete the "custom-pack" folder (following the path provided above) via FTP.</span></p>
							<?php } ?>
						</div>
