...
function blox_format_image() {
    ...
    if (strpos($image_content, "<") !== false) {
        ...
        // Image url
        echo "<img src='$image_content' alt='Featured Image'/>";
    }
    echo '</div><!-- .entry-media -->';
}
    ...
    if ($image_source!='') {
        ...
        if( in_array($overlay, array('permalink', 'lightbox', 'both')) && ($video_content=='' && $portfolio_gallery_field=='') ) {
            ...
            $result = '<div class="entry_media">
                            ...
                            <img itemprop="image" src="' . $image . '" alt="Post Image"/>
                       </div>';
        }
        else{
            ...
            if( get_post_type($tmp_post) == 'portfolio' ){
                ...
                if( $video_content !='' ){
                    ...
                    foreach ($arr as $value) {
                        if( $value!='' ){
                            $gallery_html .= '<img src="'. wp_get_attachment_url($value) .'" />';
                        }
                    }
                    $result = '<div class="entry_media">
                    ...
                    $result = '<div class="entry_media">
                                    <a href="'. permalink() .'"><img itemprop="image" src="' . $image . '" alt="Post Image"/></a>
                               </div>';
                }
            }
            else{
...
function hover_featured_image_withtitle($post_id, $placeholder = false, $w = 868, $h = 0) {
    ...
    if ($image_source!='') {
        ...
        $result .= '    <img itemprop="image" src="' . $image . '" alt="Post Image"/>
                </div><!-- end .entry_media -->';
    }
    return $result;
}
