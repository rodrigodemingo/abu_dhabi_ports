 ...
 * ADD ELEMENT MODAL
 */
function add_blox_element($container, $pos){
	var $lis = '';
	blox_iso_items = [];
	for(i=0; i<blox_items.length; i++){
		$lis += '<li class="'+blox_item_filters[i]+'" id="_'+i+'"><a href="javascript:;" type="'+blox_items[i]+'"><i class="'+(blox_item_icons[i]!='' ? blox_item_icons[i] : 'icon-star')+'"></i><span>'+blox_item_titles[i]+'</span></a></li>';
		var tmp = {};
        tmp.id = '_'+i;
	...
	$modal.append('<div class="blox_elements_toolbar"> \
						<span class="element_filter"> \
							...
							<a href="javascript:;" data-filter=".type-container">Container</a> \
							<a href="javascript:;" data-filter=".type-element">Elements</a> \
							<a href="javascript:;" data-filter=".type-media">Media</a> \
							<input type="text" id="blox_iso_item_search" placeholder="Search element..." onkeyup="isotopeSearch(jQuery(this).val().toLowerCase());" />\
							<a href="javascript:;" onclick="jQuery(\'#blox_iso_item_search\').val(\'\'); isotopeSearch(false); return false;"><i class="icon-times"></i></a> \
						</span> \
					</div>');
						...
						jQuery.post( ajaxurl, {'action':'blox_template_save', 'title':jQuery('#blox_save_template_name').val(), 'content':$content.html()}, function(data){
							if( data!='-1' ){
								...
								for(var i=0; i<$json.length; i++){
									html += '<span><a href="javascript: blox_load_template(&quot;'+$json[i].id+'&quot;);" data-template="'+$json[i].id+'">'+$json[i].title+'</a><i class="icon-times" onclick="blox_remove_template(jQuery(this));"></i></span>';
								}
...
function show_blox_form_handler($form, callback, params){
	...
	if( (typeof params!=='undefined') && (typeof params.target!=='undefined') ){
		// Skin option
		if( (typeof params.skin!=='undefined') && params.skin == true ){
			$form.append( get_bform_field_select({
				type: 'select',
	            id: 'blox_element_skin',
	            label: 'Element Skin',
	            options: [
	            	{ value: 'default', label: 'Default' },
	            	{ value: 'cleanskin', label: 'Clean Skin' }
	            ],
		...
		if( typeof params.extra_field!=='undefined' && params.extra_field == true ){
			$form.append( get_bform_field_animation({
	            ...
	            description: 'Animation element appears first.'
			}) );
			$form.append( get_bform_field_input({
	            ...
	            description: 'Add here a class name then add your custom styles on Theme Options => <b>Custom CSS</b> tab.'
			}) );
		}
	}
	...
	jQuery('#blox_popup_window').find('.blox_popup_button_update').unbind('click')
    	.click(function(){
    		...
    		if( typeof params!=='undefined' && typeof params.target!=='undefined' ){
    			if( typeof params.skin!=='undefined' && params.skin == true ){
	    			params.target.attr('skin', jQuery('#blox_element_skin').val());
	    		}
...
function get_bform_field_animation(elem){
	...
	options += '<option value="top-to-bottom">Top to Bottom</option>';
	options += '<option value="bottom-to-top">Bottom to Top</option>';
	options += '<option value="left-to-right">Left to Right</option>';
	options += '<option value="right-to-left">Right to Left</option>';
	options += '<option value="appear">Zoom in</option>';
...
function get_bform_field_editor(elem){
	...
	var $obj = jQuery('<div id="wp-blox_tinymce_editor-wrap" class="wp-core-ui wp-editor-wrap tmce-active"> \
						...
						<div id="wp-blox_tinymce_editor-editor-tools" class="wp-editor-tools hide-if-no-js"> \
							...
							<div id="wp-blox_tinymce_editor-media-buttons" class="wp-media-buttons"> \
								<a href="#" id="insert-media-button" class="button insert-media add_media" data-editor="blox_tinymce_editor" title="Add Media"> \
									<span class="wp-media-buttons-icon"></span> Add Media \
								</a> \
							</div> \
						</div> \
						<div id="wp-blox_tinymce_editor-editor-container" class="wp-editor-container"> \
							<textarea class="wp-editor-area" rows="12" cols="40" name="blox_tinymce_editor" id="blox_tinymce_editor"></textarea> \
						</div> \
					</div>');
