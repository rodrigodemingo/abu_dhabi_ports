...
function render_megamenu_items($menies, $parent){
    ...
    foreach($menies as $meny){
        if( $meny->menu_item_parent == $parent ){
            ...
            $html .= '<div id="'. get_menu_id($meny) .'" class="menu_column '.$menu_classes.'">
                        <h3>'.$meny->title.'</h3>
...
function render_megamenu_subitems($menies, $parent){
    ...
    foreach($menies as $meny){
        if( $meny->menu_item_parent == $parent ){
            ...
            $html .= '<div id="'. get_menu_id($meny) .'" class="menu_item '.$menu_classes.'">
                        <a href="'.$meny->url.'" '.get_custom_atts_menu($meny).'>
                        	'.get_icon_menu($meny).'
                            '.$meny->title.'
                            '.get_description_menu($meny).'
...
function render_mega_nav($menu_location='primary'){
    ...
    /* Checking if custom menu selected on page */
    $custommenu = tt_getmeta("pagemenu");
    $printcustom = false;
    if(isset($custommenu) && $custommenu!='' && $custommenu != 'default') {
        $printcustom = true;
    }

    if( isset($theme_locations[$menu_location]) && (int)$theme_locations[$menu_location]>0 || $printcustom){
        $menu_obj = get_term( $theme_locations[$menu_location], 'nav_menu' );
        $menies = wp_get_nav_menu_items($menu_obj);

        /* Picking up custom menu if there specified */
        if($printcustom) { $menies = wp_get_nav_menu_items($custommenu); }
    ...
    $mobile = 'mobile-menu';
    if (isset($theme_locations[$mobile]) && $theme_locations[$mobile] > 0 && $smof_data['use_responsive'] == 1) {
        wp_nav_menu(array(
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <p class="field-image description description-wide">
                <label for="edit-menu-item-image-<?php echo $item_id; ?>">
                    <?php _e( 'Background image for mega menu column', 'themeton' ); ?><br />
            ...
            <p class="field-color description description-wide">
                <label for="edit-menu-item-color-<?php echo $item_id; ?>">
                    <?php _e( 'Menu BG Color for Metro style', 'themeton' ); ?><br />
